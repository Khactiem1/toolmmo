...
export default {
  ...
  setup(props, { emit }) {
    ...
    function pointerDown(e) {
      ...
      const isAbsolute =
        window.getComputedStyle(vfmContent.value).position === "absolute";
      const position = {
          ...
          const limit = {
            ...
            relative() {
              return {
                ...
                maxTop:
                  position.top + rectContainer.bottom - rectContent.bottom,
                maxLeft:
                  position.left + rectContainer.right - rectContent.right,
              };
            },
          };
      ...
      const resetBodyCursor =
        state === STATE_RESIZE &&
        setStyle(document.body, "cursor", resizeCursor[direction]);
    ...
    function getResizeOffset(
      ...
      const setOffset = (dir) => {
        ...
        offsetAxis = props.fitParent
          ? clamp(dir.min, offsetAxis, dir.max)
          : offsetAxis;
      ...
      const getDirectionInfo = (position, edgeName, axis, isPositive) => {
        ...
        return {
          ...
          getEdge: (offsetAxis) =>
            rectContent[edgeName] - offsetAxis * (isPositive ? 1 : -1),
