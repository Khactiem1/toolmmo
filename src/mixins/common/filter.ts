...
export const filter = {
  created() {
	...
	//   Thêm 2 operator cho filter local
		contains: function (item, property, value) {
			let data = this.getValueFilter(item, property);
			let lowercaseValues = value.toLowerCase()
			let result = data.contains(lowercaseValues)
			return result;
		},
      ...
      '*.': function(item, property, value) {
        ...
        if (typeof data === 'string') {
          ...
          throw Error(
            'DEV: operator startswith only support field with data type string.'
          );
        }
      },
      ...
      '*': function(item, property, value) {
        ...
        if (typeof data === 'string') {
          ...
          throw Error(
            'DEV: operator contains only support field with data type string.'
          );
        }
      },
      '!*': function(item, property, value) {
      ...
      NULL: function(item, property, value) {
        let data = this.getValueFilter(item, property);
      ...
      'NOT NULL': function(item, property, value) {
        let data = this.getValueFilter(item, property);
        if (data === null || data === undefined || data === '') {
          return false;
        } else {
  ...
  methods: {
    ...
    testFilter(op, source, value) {
      switch (op) {
        case this.$ms.constant.FilterOperator.Contains:
          if (!value) {
            return true;
          } else if (
            source &&
          ...
          break;
        case this.$ms.constant.FilterOperator.Equals:
          if (source === value) {
            return true;
          }
          break;
    ...
    filterRecord(filter, item) {
      ...
      match = filterFn.call(me, item, property, value);
    ...
    createRegexFilter(filter) {
      ...
      return function(item) {
        var val = typeof item === 'object' ? item[property] : item;
